...
exports.formatDate = (year, month, day, hour, minute) => {
...
exports.getDate = (value) => {
    ...
    const y = date.getFullYear(), m = date.getMonth() + 1, d = date.getDate(), h = date.getHours(), //获取当前小时数(0-23)
    f = date.getMinutes();
    return exports.formatDate(y, m, d, h, f);
};
// 获取对应年份月份的天数
exports.getMonthDay = (year, month) => {
    var d = new Date(year, month, 0);
    return d.getDate();
};
//根据时间2019-01-02 09：12  得到 ['2019','1','2','9','12']
exports.getArrWithTime = (str) => {
    let arr1 = str.split(' ');
...
// 获取最近的年、月、日、时、分的集合
exports.getPickerViewList = () => {
    const now = new Date();
    const year = now.getFullYear();
    const month = now.getMonth() + 1;
    const yearList = [];
    const monthLsit = [];
    const dayList = exports.getDayList(year, month);
    const hourList = [];
    const minuteList = [];
